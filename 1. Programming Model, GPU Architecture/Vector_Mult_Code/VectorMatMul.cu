...
__global__ void MatMulGPU( float *a, float *b, float *c, int row,int col, int k ) { // must be pointers in GPU memory
    ...
    if ((tidx < col) && (tidy < row)){   // All threads in wrap execute same code. We don't want threads with tid > vector_size to run this code. tid > vector_size -> this condition will be true for certain threads in *last* wrap
        float sum=0;
        for (int z=0;z<k;++z){
