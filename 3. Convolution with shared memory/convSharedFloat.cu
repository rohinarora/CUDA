...
__global__ void convShared(float *in, float *out, int vector_size) {
    __shared__ float temp[BLOCK_SIZE + 2 * RADIUS]; //copy all the data the block would need into shared memory first. img2.png
...
int main( int argc, char* argv[] ) { 
	...
	float *out_gpu_on_cpu = new float [vector_size]; // Copy GPU output to CPU
	...
	float *out_gpu;
	cudaMalloc (&in_gpu, vector_size*sizeof(float));
	cudaMalloc (&out_gpu, vector_size*sizeof(float));
	cudaMemcpy (in_gpu, in_cpu, vector_size*sizeof(float), cudaMemcpyHostToDevice);
	...
	cudaMemcpy (out_gpu_on_cpu, out_gpu, vector_size*sizeof(float), cudaMemcpyDeviceToHost); // copy the array 'c' back from the GPU to the CPU
